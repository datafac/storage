...
namespace DTOMaker.CSRecord.Tests
{
    public class PolymorphismTests
    {
        ...
        public async Task Polymorphic02_Equilateral()
        {
            ...
            source.HintName.ShouldBe("MyOrg.Models.Equilateral.CSRecord.g.cs");
        ...
        public async Task Polymorphic03_Polygon()
        {
            ...
            source.HintName.ShouldBe("MyOrg.Models.Polygon.CSRecord.g.cs");
            string outputCode = string.Join(Environment.NewLine, source.SourceText.Lines.Select(tl => tl.ToString()));
            await Verifier.Verify(outputCode);
        }
        ...
        public async Task Polymorphic04_Quadrilateral()
        {
            ...
            source.HintName.ShouldBe("MyOrg.Models.Quadrilateral.CSRecord.g.cs");
            string outputCode = string.Join(Environment.NewLine, source.SourceText.Lines.Select(tl => tl.ToString()));
            await Verifier.Verify(outputCode);
        }
        ...
        public async Task Polymorphic05_Rectangle()
        {
            ...
            source.HintName.ShouldBe("MyOrg.Models.Rectangle.CSRecord.g.cs");
            string outputCode = string.Join(Environment.NewLine, source.SourceText.Lines.Select(tl => tl.ToString()));
            await Verifier.Verify(outputCode);
        }
        ...
        public async Task Polymorphic06_RightTriangle()
        {
            ...
            source.HintName.ShouldBe("MyOrg.Models.RightTriangle.CSRecord.g.cs");
            string outputCode = string.Join(Environment.NewLine, source.SourceText.Lines.Select(tl => tl.ToString()));
            await Verifier.Verify(outputCode);
        }
        ...
        public async Task Polymorphic07_Square()
        {
            ...
            source.HintName.ShouldBe("MyOrg.Models.Square.CSRecord.g.cs");
            string outputCode = string.Join(Environment.NewLine, source.SourceText.Lines.Select(tl => tl.ToString()));
            await Verifier.Verify(outputCode);
        }
        ...
        public async Task Polymorphic08_Triangle()
        {
            ...
            source.HintName.ShouldBe("MyOrg.Models.Triangle.CSRecord.g.cs");
            string outputCode = string.Join(Environment.NewLine, source.SourceText.Lines.Select(tl => tl.ToString()));
            await Verifier.Verify(outputCode);
        }
    }
